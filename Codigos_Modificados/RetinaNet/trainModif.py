...
def main(args=None):
    ...
    parser.add_argument('--model',help='path to .pt file',type=str, default='')
    ...
    if parser.model =='':
        ...
        load_model=torch.load(parser.model)
        state_dict=load_model.state_dict()
        state_dict_nuevo={}
        for key,value in state_dict.items():
            key_n=key.replace("module.","")
            if key_n != 'classificationModel.output.weight' and key_n != 'classificationModel.output.bias':
                state_dict_nuevo[key_n]=value
            if key_n == 'classificationModel.output.weight':
                aleatorio = torch.rand(9*dataset_train.num_classes(), value.size(1), value.size(2), value.size(3))
                aleatorio[:9*dataset_train.num_classes() - value.size(0)] = value
                state_dict_nuevo[key_n]=aleatorio
            if key_n == 'classificationModel.output.bias':
                aleatorio = torch.rand(9*dataset_train.num_classes())
                aleatorio[:9*dataset_train.num_classes() - value.size(0)] = value
                state_dict_nuevo[key_n]=aleatorio         

        retinanet.load_state_dict(state_dict_nuevo)
